...
!bloss(072009): Note that o3, etc. are now in ppmv and don't need conversions.
    ...
    o3vmr(:, 1:nzm)    = spread(o3(:), dim = 1, ncopies = nx) 
    co2vmr(:, 1:nzm)   = spread(co2(:), dim = 1, ncopies = nx) 
    ch4vmr(:, 1:nzm)   = spread(ch4(:), dim = 1, ncopies = nx) 
    n2ovmr(:, 1:nzm)   = spread(n2o(:), dim = 1, ncopies = nx) 
    o2vmr(:, 1:nzm)    = spread(o2(:), dim = 1, ncopies = nx) 
    cfc11vmr(:, 1:nzm) = spread(cfc11(:), dim = 1, ncopies = nx) 
    cfc12vmr(:, 1:nzm) = spread(cfc12(:), dim = 1, ncopies = nx) 
    cfc22vmr(:, 1:nzm) = spread(cfc22(:), dim = 1, ncopies = nx) 
    ccl4vmr(:, 1:nzm)  = spread(ccl4(:), dim = 1, ncopies = nx)
    
!tcram: extrapolate into extra layer at top
    o3vmr(:, nzm+1)    = spread(o3(nzm), dim = 1, ncopies = nx) 
    co2vmr(:, nzm+1)   = spread(co2(nzm), dim = 1, ncopies = nx)
    ch4vmr(:, nzm+1)   = spread(ch4(nzm), dim = 1, ncopies = nx)
    n2ovmr(:, nzm+1)   = spread(n2o(nzm), dim = 1, ncopies = nx) 
    o2vmr(:, nzm+1)    = spread(o2(nzm), dim = 1, ncopies = nx) 
    cfc11vmr(:, nzm+1) = spread(cfc11(nzm), dim = 1, ncopies = nx) 
    cfc12vmr(:, nzm+1) = spread(cfc12(nzm), dim = 1, ncopies = nx) 
    cfc22vmr(:, nzm+1) = spread(cfc22(nzm), dim = 1, ncopies = nx) 
    ccl4vmr(:, nzm+1)  = spread(ccl4(nzm), dim = 1, ncopies = nx)
